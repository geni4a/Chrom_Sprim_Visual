    ...
    for file in os.listdir(sprime_files):
        for name in populations:
            if name in file: 
                ...
                # print(file)
                # print(dlfkjdfjldkjf)
                df_sorted = df.sort_values(by=["CHROM", "POS"], ascending=True)
                df_sorted = df_sorted[df_sorted["CHROM"] != "nan"]
                df_sorted[df.columns[0]] = df_sorted["CHROM"].astype(int)
                df_sorted["POS"] = df_sorted["POS"].astype(int)
                df_sorted["SEGMENT"] = df_sorted["SEGMENT"].astype(int)
                df_sorted = df_sorted.sort_values(by=["CHROM", "POS", "SEGMENT"], ascending=True)
...
def make_chrom_dict():
    for i in range(1, 23):
        ...
        for pop in populations:
            ...
            merged_df = pd.merge(df, result_df, on='SEGMENT', how='left')
            # print(merged_df)
            pop_chrom_dict[i][pop] = merged_df
# make_chrom_dict()
# print(pop_chrom_dict[1].keys())
...
# print(len(list(concatenated_dict.keys())))
...
def make_files(path1, path2, path3):
    ...
    calc_sharing()
    print("Done 10/10")
    return populations, ov_num_dict, pop_chrom_dict, pop_dataf, concatenated_dict, ov_dict
